package com.suda.mychatapp.utils.gif;
...
public class AnimatedGifDrawable extends AnimationDrawable {
    ...
    public AnimatedGifDrawable(InputStream source, UpdateListener listener, int size) {
        mListener = listener;
        ...
        for (int i = 0; i < decoder.getFrameCount(); i++) {
            ...
            drawable.setBounds(0, 0, bitmap.getWidth() + size, bitmap.getHeight() + size);
            addFrame(drawable, decoder.getDelay(i));
            if (i == 0) {
                // Also set the bounds for this container drawable
                setBounds(0, 0, bitmap.getWidth() + size, bitmap.getHeight() + size);
            }
        }
    }
